...
@DataBoundConstructor: 标注构造函数来进行数据绑定。用于构造类的实例，将请求参数绑定到对象属性上；
@DataBoundSetter: 标注 setter 方法来进行数据绑定。用于设置对象属性，将请求参数绑定到对象属性上；
@DataBoundMethod: 标注在一个类中的所有方法都可以进行数据绑定。可以避免在每个方法上都标注 @DataBoundConstructor 或 @DataBoundSetter；
...
jianbo li
继续
一个源代码管理器。Jenkins 中支持多种 SCM 工具，如 Git、SVN 等。通过该类可以获取 SCM 工具的各种属性，如 URL、分支、目录等。
